...
export default function CategoryPage({ params }: { params: { category: string } }) {
  ...
  const categoryName = categorySlug ? categorySlug.charAt(0).toUpperCase() + categorySlug.slice(1) : 'Unknown'
    ...
    return (
      <div className="min-h-screen bg-stone-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-stone-600 mb-8">The category "{categoryName}" doesn't exist.</p>
          <Link href="/" className="bg-gradient-to-r from-amber-600 to-orange-600 text-white px-8 py-3 rounded-lg font-semibold hover:from-amber-700 hover:to-orange-700 transition-all">
            Back to Home
          </Link>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              <div className="flex items-center space-x-3 mb-4">
                ...
                <span className="text-amber-300 font-semibold tracking-wide">CATEGORY</span>
              </div>
              ...
              <p className="text-xl text-stone-300 leading-relaxed mb-8">
                ...
                From cutting-edge technology to innovative designs, find everything you need to elevate your experience.
              </p>
              ...
              <div className="grid grid-cols-3 gap-6 pt-6 border-t border-stone-700">
                ...
                <div className="text-center">
                  <div className="text-2xl font-bold text-amber-400">
                    {(categoryProducts.reduce((acc, p) => acc + (p.rating || 0), 0) / categoryProducts.length).toFixed(1)}★
                  </div>
              ...
              <div className="bg-gradient-to-br from-stone-800 to-amber-900 rounded-3xl p-8 shadow-2xl border border-amber-700 border-opacity-30">
                <div className="grid grid-cols-2 gap-4">
                  {categoryProducts.slice(0, 4).map((product, index) => (
                    <div key={product.id} className={`bg-gradient-to-br from-amber-600 to-orange-700 rounded-xl p-4 text-center shadow-lg transform hover:scale-105 transition-all duration-300 ${index % 2 === 1 ? 'mt-4' : ''}`}>
                      <img 
                        src={product.image} 
                        alt={product.name}
                      ...
                      <div className="text-amber-200 text-xs">₦{(product.price / 100).toLocaleString()}</div>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
              <div className="flex items-center space-x-4">
                ...
                <p className="text-stone-600">
                  Showing <span className="font-semibold text-stone-900">{filteredProducts.length}</span> of <span className="font-semibold text-stone-900">{categoryProducts.length}</span> products
                </p>
              </div>
                ...
                <div className="mb-8">
                  ...
                  <div className="space-y-3">
                    ...
                    <input
                      ...
                      style={{
                        background: `linear-gradient(to right, #d97706 0%, #d97706 ${(priceRange[1] / 300000) * 100}%, #e5e7eb ${(priceRange[1] / 300000) * 100}%, #e5e7eb 100%)`
                      }}
                    />
                  </div>
                </div>
            ...
            <div className="lg:col-span-3">
              {filteredProducts.length > 0 ? (
                ...
                <div className="text-center py-16">
                  ...
                  <h3 className="text-2xl font-bold text-stone-900 mb-4">No Products Found</h3>
                  <p className="text-stone-600 mb-8">Try adjusting your filters to see more products.</p>
                  <button
                    onClick={() => {
                      setPriceRange([0, 300000])
                      setSelectedBrands([])
      ...
      <section className="py-16 bg-white">
        <div className="max-w-7xl mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold text-stone-900 mb-4">Explore Other Categories</h2>
            <p className="text-lg text-stone-600">Discover more amazing products</p>
          </div>

          <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            {[...new Set(products.map(p => p.category))].filter(cat => cat.toLowerCase() !== categorySlug.toLowerCase()).map(category => {
              ...
              return (
                ...
                >
                  ...
                  <p className="text-sm text-stone-600 mb-2">{categoryProducts.length} products</p>
